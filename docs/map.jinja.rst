...
We will see a quick setup to configure the ``unifios`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``unifios`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``unifios`` formulas under ``/srv/salt/unifios/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/unifios/parameters/dns:domain/
    ln -s dns:domain /srv/salt/unifios/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/unifios/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/unifios/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``unifios`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``unifios`` formulas under ``/srv/salt/unifios/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/unifios/parameters/roles
...
We create a configuration for the role ``unifios/server`` in ``/srv/salt/unifios/parameters/roles/unifios/server.yaml``:
...
We create another configuration for the role ``unifios/client`` in ``/srv/salt/unifios/parameters/roles/unifios/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles unifios/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles unifios/client
...
.. code-block:: console
    ...
    mv /srv/salt/unifios/parameters/dns:domain/  '/srv/salt/unifios/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-unifios-as-json:
      test.nop:
        - name: {{ unifios | json }}
...
Here is an example based on `template-formula/unifios/config/file.sls`_:
    ...
    unifios-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='unifios-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ unifios.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            unifios: {{ unifios | json }}

This ``sls`` file expose a ``unifios`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ unifios['winner'] }}
...
.. _template-formula/unifios/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/unifios/config/file.sls
